import minimalmodbus # requires modified version
...
# 	import time
# 	ps = ElectroAutomatikGmbHPowerSupply('COM27')
...
def connect_PSU():
	global ps
	ps = ElectroAutomatikGmbHPowerSupply('COM13')
	ps.set_voltage_value = 0.0
	ps.set_current_value = 20.0
	ps.remote_mode(True)
	ps.enable_output(True)
...
def disconnect_PSU():
	global ps
	ps.set_voltage_value = 0.0
	ps.set_current_value = 0.0
	ps.enable_output(False)
	ps.remote_mode(False)


def set_PSU_VOLTAGE(volt):
	global ps
	ps.set_voltage_value = volt

def read_PSU_Volt_Amp():
	global ps
	# print(f'Measured voltage and current is: {ps.measured_voltage:.2f} V, {ps.measured_current:.2f} A')
	return ps.measured_voltage, ps.measured_current